...
type OriginRule = {
  ...
  maxMinutes?: number;               // e.g., 15
  maxDistanceKm?: number;            // e.g., 12
  mode?: Mode;                       // optional per-origin override
};

/** Distance Matrix minutes for a single origin against N destinations (batched) */
async function dmMinutesForOrigin(
  originLonLat: [number, number],
  destinationsLonLat: [number, number][],
  mode: Mode
): Promise<(number | null)[]> {
  const origin = `${originLonLat[1]},${originLonLat[0]}`;
  const out: (number | null)[] = [];

  for (let i = 0; i < destinationsLonLat.length; i += 25) {
    const chunk = destinationsLonLat.slice(i, i + 25);
    const destinations = chunk.map(c => `${c[1]},${c[0]}`).join('|');
    const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${origin}&destinations=${destinations}&mode=${mode}&key=${GOOGLE_API_KEY}`;
    const { data } = await axios.get(url);
    const els = data?.rows?.[0]?.elements || [];
    for (const el of els) {
      const secs = el?.duration?.value;
      out.push(Number.isFinite(secs) ? secs / 60 : null);
    }
  }
  return out;
}

function buildBaseMatch(userProfile: any, filters: any) {
  const matchStage: any = {
    countryId: new mongoose.Types.ObjectId(userProfile.country),
    userProfileId: { $ne: userProfile._id },
    responders: { $ne: userProfile._id },
...
export const getAllLeadFromDB = async (
  ...
  const userlocation= await UserLocationServiceMap.find({userProfileId:userProfile._id}).populate('locationGroupId');
 
// const originsConfigTest = userlocation.map((loc) => {
//   const { locationType, traveltime, travelmode, rangeInKm, locationGroupId } = loc;

//   // Extract coordinates (convert to numbers)
//   const lon = locationGroupId?.longitude ? Number(locationGroupId.longitude) : null;
//   const lat = locationGroupId?.latitude ? Number(locationGroupId.latitude) : null;

//   if (locationType === 'travel_time') {
//     return {
//       coord: [lon, lat],
//       maxMinutes: Number(traveltime),
...
//     return {
//       coord: [lon, lat],
//       maxDistanceKm: Number(rangeInKm),
//     };
//   }
...
// console.log('originsConfigTest:', originsConfigTest);


let originsConfigsTest: [
  { coord: [ -26.5821, 152.9512], maxMinutes: 5, mode: 'walking' },
  // { coord: [151.2041, -33.8928], maxDistanceKm: 100 },
  // { coord: null, nationWide: true }
]
  ...
  const baseMatch = buildBaseMatch(userProfile, filters);

//  console.log("originsConfigTest?.originsConfig",originsConfigTest)
  // NEW: per-origin rules array
  const originsConfig: OriginRule[] = Array.isArray(originsConfigsTest) ? originsConfigsTest : [];
  // const originsConfig: OriginRule[] = Array.isArray(filters?.originsConfig) ? filters.originsConfig : [];



  
  const defaultMode: Mode = filters?.mode || 'driving';
  const useGeoTime = originsConfig.length > 0;

  let allowedIds: Set<string> | null = null;
  const bestMinutesById = new Map<string, number>();
  const bestDistanceKmById = new Map<string, number>();

  if (useGeoTime) {
    for (const rule of originsConfig) {
      const origin = rule.coord;
      const mode: Mode = rule.mode || defaultMode;

      // Distance-only rule → single $geoNear pass
      if (typeof rule.maxDistanceKm === 'number') {
        const maxMeters = rule.maxDistanceKm * 1000;
        const prePipeline: any[] = [
          ...
          { $match: baseMatch },
          { $project: { _id: 1, location: 1, distanceMeters: 1 } },
          { $limit: 2000 }
        ];
        const rows = await Lead.aggregate(prePipeline);
        for (const r of rows) {
          const id = r._id.toString();
          if (!allowedIds) allowedIds = new Set();
          allowedIds.add(id);
          const distKm = (r.distanceMeters ?? 0) / 1000;
          const prev = bestDistanceKmById.get(id);
          if (prev === undefined || distKm < prev) bestDistanceKmById.set(id, distKm);
        }
        continue;
      }

      // Time rule → radius prefilter then Distance Matrix minutes
      if (typeof rule.maxMinutes === 'number') {
        // generous prefilter radius to limit API calls (e.g., 25km)
        const preRadiusMeters = 25000;
        const prePipeline: any[] = [
          {
            $geoNear: {
              near: { type: 'Point', coordinates: origin },
              distanceField: 'distanceMeters',
              maxDistance: preRadiusMeters,
              spherical: true,
            }
          },
          { $match: baseMatch },
        ...
        const preList = await Lead.aggregate(prePipeline);
        if (!preList.length) continue;

        const dests = preList.map((d: any) => d.location.coordinates as [number, number]);
        const minutes = await dmMinutesForOrigin(origin, dests, mode);

        for (let i = 0; i < preList.length; i++) {
          const mins = minutes[i];
          if (mins !== null && mins <= rule.maxMinutes!) {
            const id = preList[i]._id.toString();
            if (!allowedIds) allowedIds = new Set();
            allowedIds.add(id);
            const prev = bestMinutesById.get(id);
            if (prev === undefined || mins < prev) bestMinutesById.set(id, mins);

            // record best straight-line distance too (from $geoNear result)
            const distKm = (preList[i].distanceMeters ?? 0) / 1000;
            const prevD = bestDistanceKmById.get(id);
            if (prevD === undefined || distKm < prevD) bestDistanceKmById.set(id, distKm);
          }
        }
      }
    }
  ...
  // Details + lookups + keyword + pagination
  const detailsMatch: any = useGeoTime
    ? { ...baseMatch, _id: { $in: Array.from(allowedIds!).map(id => new mongoose.Types.ObjectId(id)) } }
    : { ...baseMatch };
  ...
  // Attach best metrics (if used)
  if (useGeoTime && data.length) {
    data = data.map((doc: any) => ({
      ...doc,
      travelTimeMinutes: bestMinutesById.get(doc._id.toString()) ?? null,
      distanceKm: bestDistanceKmById.get(doc._id.toString()) ?? null,
    }));
  }
